...
| `theme` | — | `dark` | `dark` または `light` |
| `badge` | — | `1` | `0` で「Listen on Apple Music」バッジを非表示 |
| `ui` | — | _(New)_ | `classic` でアルバムカードと同じレイアウト（左にアートワーク＋「番号・曲名・時間」の1カラム）で描画。`desc`・`limit` は無視 |
| `limit` | — | `7` | 表示するトラック数（New レイアウトのみ）。`all` で取得済みの全曲。スクレイプ取得数にクランプ。不正値や0以下は `7` にフォールバック |
...
- New レイアウトは `limit` 曲（既定7）を表示し、超過分は「他N曲…」として要約します。Classic レイアウトは常に最大7曲。
...
- **フル** — `url=` にパーセントエンコードした `music.apple.com` リンクを渡す。
- **コンパクト**（推奨） — `id` + `s` を渡すと、サーバが `/{kind}/_/{id}` 形式
  （プレースホルダのスラッグ。Apple は id で解決してフル URL へリダイレクト）に再構築
  します。リンク全体のパーセントエンコードが不要になり、長さがおよそ半分になります。
  プレースホルダによりデスクトップアプリの `itms://` ハンドラが期待するパス構造を保つ
  ため、アプリのディープリンクも正しく動作します。

### フロー

1. リクエストが `music.apple.com` 上の `https` リンクに解決できること（有効な `url`、
   または `id`/`s` の組）。それ以外は `400`。
...
`url`、または コンパクトな `id` + `s` のどちらかを指定します。
